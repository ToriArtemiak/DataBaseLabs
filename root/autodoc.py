...
def _rule_params(rule_str: str):
    ...
    for m in _param_re.finditer(rule_str):
        ...
        out.append({"in": "path", "name": name, "required": True, "schema": schema})
    return out
...
def openapi_json():
    ...
    for rule in current_app.url_map.iter_rules():
        ...
        if not allowed:
            continue
        openp = _param_re.sub(lambda m: "{" + m.group("name") + "}", path)
        params = _rule_params(path)
        entry = paths.setdefault(openp, {})
        for m in sorted(allowed):
            entry[m.lower()] = _op_obj(openp, m, params)
