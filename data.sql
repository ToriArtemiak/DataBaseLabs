...
DROP TRIGGER IF EXISTS before_update_review;
...
CREATE TABLE IF NOT EXISTS `dealerships` (
  ...
  INDEX `idx_dealership_name` (`name`)
) ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `sellers` (
  ...
  INDEX `idx_seller_dealership` (`dealership_id`),
...
INSERT INTO sellers (name, dealership_id) VALUES
('Mike Johnson',1),('Alice Cooper',2),('Steve Rogers',3),('Nataly Jennings',4),('Bruce Wayne',5),
('Clark Kent',6),('Diana Prince',7),('Barry Allen',8),('Arthur Curry',9),('Victor Stone',10);

INSERT INTO car_types (type_name) VALUES
('Sedan'),('SUV'),('Truck'),('Convertible'),('Coupe'),
('Hatchback'),('Minivan'),('Wagon'),('Electric'),('Diesel');

INSERT INTO cars (model, brand, price, country, mileage, year, car_type_id, seller_id, dealership_id) VALUES
...
('Color'),('Transmission'),('Fuel Type'),('Sunroof'),('GPS'),
('Leather Seats'),('Heated Seats'),('Bluetooth'),('Backup Camera'),('Alloy Wheels');

INSERT INTO feature_options (feature_id, option_value) VALUES
(1,'Red'),(1,'Blue'),(1,'Black'),(1,'White'),
(2,'Automatic'),(2,'Manual'),
(3,'Electric'),(3,'Gasoline'),(3,'Diesel'),(3,'Hybrid');

INSERT INTO car_feature_values (car_id, feature_id, option_id) VALUES
(1,1,1),(1,2,5),(1,3,7),
(2,1,2),(2,2,6),(2,3,8),
(3,1,3),(3,2,6),(3,3,9),
(4,1,4);
...
INSERT INTO comments (car_id, user_id, comment_text, created_at) VALUES
(1,1,'Amazing car!',NOW()),(2,2,'Very fuel efficient.',NOW()),
(3,3,'Perfect for long trips.',NOW()),(4,4,'Comfortable and reliable.',NOW()),
(5,5,'Great performance!',NOW()),(6,6,'Affordable and practical.',NOW()),
(7,7,'Perfect for off-road adventures.',NOW()),(8,8,'Environmentally friendly.',NOW()),
(9,9,'Great value for the price.',NOW()),(10,10,'Ideal for family use.',NOW());
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM cars WHERE id = NEW.car_id) THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Car ID does not exist in cars table';
  END IF;
END $$
DELIMITER ;
...
INSERT INTO reviews (car_id, review_text, rating) VALUES (1, 'Excellent car, very reliable!', 5);
